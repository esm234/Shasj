...
# --- استدعاء المحلل الذكي الجديد والمولد ---
...
# ... (باقي دوال الأوامر الإدارية مثل export, import, ban, etc.)

# --- معالج الرسائل الرئيسي (معدل لاستخدام AI) ---
async def handle_user_message(update: Update, context: CallbackContext) -> None:
    ...
    if message.text:
        ...
        if raw_content: text_to_parse = raw_content
    elif message.video:
        message_type, raw_content, file_id = "فيديو", message.caption or "", message.video.file_id
        if raw_content: text_to_parse = raw_content
    # ... (يمكن إضافة باقي أنواع الوسائط بنفس الطريقة)
    else: # رسائل بدون نص لتحليله
        if message.voice: message_type, file_id = "رسالة صوتية", message.voice.file_id
    ...
    question_data = {
        'question_id': question_id, 'user_id': user.id, 'username': user.username or "", 
        'fullname': user.full_name, 'message_type': message_type, 
        'timestamp': datetime.now().isoformat(), 'message_id': message.message_id, 
        'file_id': file_id, 'raw_content': raw_content
    }

    if parsed_data:
        logger.info(f"AI returned: {parsed_data}")
        question_data['question_text'] = parsed_data.get('question_text')
        question_data['options'] = parsed_data.get('options', [])
    else: # خطة بديلة في حال فشل الـ AI أو عدم وجود نص
        question_data['question_text'] = raw_content if not file_id else ""
        question_data['options'] = []

    questions_data[question_id] = question_data
    save_data(questions_data, DATA_FILE)
    
    # ... (باقي كود الدالة لتحديث بيانات المستخدم وإرسال الرسائل)
...
# ... (باقي الدوال مثل forward_to_admin, handle_user_reply, etc.)
...
async def forward_to_admin_group_new(context: CallbackContext, q_data: Dict):
    ...
    try:
        ...
        # ... (باقي كود الإرسال للجروب)
        if q_data['message_type'] == "نص":
...
async def setup_commands(application: Application) -> None:
    ...
    admin_commands = [
        ...
        BotCommand("banned", "📋 المحظورين"), BotCommand("makepdf", "📄 إنشاء PDF للأسئلة")
    ]
